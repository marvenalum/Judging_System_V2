    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <div class="p-6">
                    <form method="POST" action="{{ route('participant.criteria.update', $criterion) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <select name="event_id" 
                                ...
                                @foreach($events as $event)
                                    <option value="{{ $event->id }}" {{ old('event_id', $criterion->event_id) == $event->id ? 'selected' : '' }}>{{ $event->event_name }}</option>
                                @endforeach
                            </select>
                            @error('event_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="category_id" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $criterion->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" 
                                   name="max_score" 
                                   id="max_score" 
                                   value="{{ old('max_score', $criterion->max_score) }}"
                                   min="0"
                                   step="0.01"
