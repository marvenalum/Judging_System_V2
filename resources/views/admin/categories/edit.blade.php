                    ...
                    <form method="POST" action="{{ route('admin.category.update', $category) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="name" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" name="name" required>
                                ...
                                <option value="talent" {{ $category->name == 'talent' ? 'selected' : '' }}>Talent</option>
                                <option value="Q&A" {{ $category->name == 'Q&A' ? 'selected' : '' }}>Q&A</option>
                                <option value="presentation" {{ $category->name == 'presentation' ? 'selected' : '' }}>Presentation</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" name="description" rows="4">{{ $category->description }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="event_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" name="event_id" required>
                                ...
                                @foreach(\App\Models\Event::all() as $event)
                                    <option value="{{ $event->id }}" {{ $category->event_id == $event->id ? 'selected' : '' }}>{{ $event->event_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="percentage_weight" class="block mt-1 w-full" type="number" name="percentage_weight" :value="$category->percentage_weight" min="0" max="100" step="0.01" />
