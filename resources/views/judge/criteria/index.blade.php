...
<div class="page">
  @if(session('success'))
    <div class="session-alert session-success">
      {{ session('success') }}
    </div>
  @endif

  @if(session('error'))
    <div class="session-alert session-error">
      {{ session('error') }}
    </div>
  @endif

  @if(session('message'))
    <div class="session-alert session-message">
      {{ session('message') }}
    </div>
  @endif
  ...
  <div class="topbar">
    <div class="topbar-left">
      ...
      <p>Active criteria for participant evaluation</p>
    </div>
  ...
  @if($criteria->isEmpty())
    <div class="card">
      <div class="empty">
        ...
        <p>No active criteria available. Check back later.</p>
      </div>
    </div>
  @else
    <!-- Table card -->
        ...
        <table>
          ...
          <tbody>
            ...
            <tr style="animation-delay: {{ $index * 0.05 }}s">
              ...
              <td><div class="cell-desc" title="{{ $criterion->description }}">{{ $criterion->description ?? 'No description' }}</div></td>
              <td>
                <span class="weight-num">{{ $criterion->max_score ?? 'N/A' }}</span>
              </td>
              <td>
...
@if($criterion->status === 'active')
              ...
              <td>
                <div class="actions">
                  <a href="{{ route('judge.criteria.show', $criterion) }}" class="btn-icon btn-view">
                    <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    View
                  </a>
                  @if($criterion->status === 'active')
                    <a href="{{ route('judge.criteria.createScore', $criterion) }}" class="btn-icon btn-score">
